# OC14 — Known issues & pre-GPU audit (2026-06-29)

> **Resolution (2026-06-29):** the headline 0.813 was **retracted** (eval→train leak + sampled decoding + noisy non-consensus rows). Leak-free greedy eval gave 0.653 (différée-starved), then the différée-restore fix (relax E1 to n_agree>=2 + vignettes ×8) gave the honest **0.822 (SFT v9, served)**. The earlier ±0.04 prediction was an underestimate — leak + sampling + noisy *différée* compounded across more than one band. Findings below kept as the audit record; ✅ = applied.

> REPORT_LIMITATIONS.md is the report-ready distillation of this audit.
...
- ⬜ Deferred **E6/E7/E8 [LOW]** Mistral retry too narrow (HTTPError-only); OpenAI cache-read priced at Anthropic's
...
- ✅ **S2** tests assert on the stale `processed/sft_train.jsonl`, not the shipped `kaggle_upload/*` → the
  ...
  0 gold∩train, 0 dup (user,assistant), all 11 vignettes present. **Resolved:** tests now assert on
  `kaggle_upload/` (the shipped artifact) and guard E2/E4/E5.

## Executed before the v9 GPU run
1. **E1 + E2 + E4** (shared registry / filter) → rebuilt the data. 2. **S2** test guards. 3. **E5, E3, M2**
into `EVAL_CELLS`/`build_sft.py`. 4. **O1 + O2 + S1** (one eval-cell pass). 5. **M1 + M3** reporting,
**O3**. Deferred E6/E7, M4/M5, O4.

Then on GPU: rebuilt clean → re-ran SFT + (batched greedy) eval → naive-Base baseline (same harness) → DPO.
This produced the leak-free **0.653** intermediate, then the *différée*-restore fix (relax E1 to
`n_agree>=2` + vignettes ×8) recovered the low-acuity class and gave the served **0.822 (SFT v9)**.
